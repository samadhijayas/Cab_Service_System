...
//   $usernic = $_SESSION['username'];
  $user_list = '';
//   $nic4='';
  $query = "SELECT * FROM driverreg ORDER BY compName";
  ...
  if ($users) {
    while ($user = mysqli_fetch_assoc($users)) {
      ...
      // $email= $user['email'];
      $user_list.="<tr>";
      $user_list.="<td>{$user['compName']}</td>";
      $user_list.="<td>{$user['tele']}</td>";      
      $user_list.="<td>{$user['email']}</td>";//    edit-user.php?user_id={$user['nic']}
      $user_list.="<td><a href='delete-driver.php?user_id={$user['email']}'>Delete</a></td>";
      $user_list.="</tr>";
